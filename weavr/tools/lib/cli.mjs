...
import { applyTargets, cancelTargets, changeTargets, finishDeployment, makeDeposit, parseMix, requestWithdraw, signChecked, transactionsFromFile, weavrClient } from './weavr.mjs';

const HERE = dirname(fileURLToPath(import.meta.url));

export const USAGE = '--address | --deployment <id> | --deposit <ticker> --amount <usd> | --withdraw <ticker> --shares <n|all> | --propose <ticker> --mix BTC:60,SOL:40 | --apply <ticker> | --cancel <ticker> | --file <walletPayload.json> [--send | --await <deploymentId>] | --tx <encoded>...';
...
export async function runCli(argv, makeSigner, env = process.env) {
  ...
  try {
    ...
    if (has('--withdraw')) {
      const portfolio = val('--withdraw');
      if (!portfolio || !val('--shares')) return fail('USAGE', '--withdraw <ticker> --shares <n|all>', EXIT.USAGE);
      return finish(await requestWithdraw(client, portfolio, val('--shares'), signer, allowed));
    }
    if (has('--propose')) {
      const portfolio = val('--propose');
      if (!portfolio || !val('--mix')) return fail('USAGE', '--propose <ticker> --mix BTC:60,SOL:40', EXIT.USAGE);
      return finish(await changeTargets(client, portfolio, parseMix(val('--mix')), signer, allowed));
    }
    if (has('--apply')) {
      const portfolio = val('--apply');
      if (!portfolio) return fail('USAGE', '--apply <ticker>', EXIT.USAGE);
      return finish(await applyTargets(client, portfolio, signer, allowed));
    }
    if (has('--cancel')) {
      const portfolio = val('--cancel');
      if (!portfolio) return fail('USAGE', '--cancel <ticker>', EXIT.USAGE);
      return finish(await cancelTargets(client, portfolio, signer, allowed));
    }
